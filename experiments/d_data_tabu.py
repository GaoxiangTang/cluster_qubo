...
for _ in tqdm(range(800)):  # tqdm will display the progress
    ...
    for qubit_size in range(10, 25):
        res_impact = hybrid_qubo_solve(qubo_matrix, max_qubit_size=qubit_size, simulated=True, group_method='impact', classical_method='tabu')
        # print('impact', res_impact['val'])
        
        res_cluster_concat = hybrid_qubo_solve(qubo_matrix, simulated=True, max_qubit_size=qubit_size, group_method='cluster', classical_method='tabu')
        # print('cluster', res_cluster_concat['val'])
        res_pool = hybrid_qubo_solve(qubo_matrix, max_qubit_size=qubit_size, group_method='pool', classical_method='tabu', simulated=True)
